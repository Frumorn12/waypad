...
Current invites may include `address`, `lan_address`, and `remote_address`.
Android treats these as connection candidates. `remote_address` is tried first
for mobile-data/direct-public use, then LAN addresses are tried as fallback.
...
Keyboard events carry XKB keysyms plus pressed/released state. Controller axes are normalized to `[-1.0, 1.0]` with Android motion-range deadzones applied before transport. Realtime axis updates are coalesced to the latest value per axis under backlog so stale stick positions are dropped instead of delivered in bursts. The host capability field `external_input.controller` is authoritative; on Linux it is true when the daemon can expose a `uinput` virtual gamepad.
