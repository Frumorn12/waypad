...
They carry host name, address, port, fingerprint, one-time code, expiry, and
route type. Android accepts the invite from an intent or pasted payload, creates
a provisional discovered host, fills the pairing code, and performs the normal
signed handshake and host-key pinning.
